...
async def _(
    event: Union[GroupDecreaseNoticeEvent, ChannelDestroyedNoticeEvent], bot: Bot
):
    """有人退群时发通知"""
    if isinstance(event, GroupDecreaseNoticeEvent):
        ...
        if event.self_id != event.user_id:
            if await db.get_guild_decrease_notice(
                event.guild_id, event.channel_id, bot.self_id
            ):
                await group_decrease_notice.finish("呜～，有人跑了")
