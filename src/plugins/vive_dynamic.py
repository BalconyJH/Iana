...
vive = on_command(
    "查看动态", aliases={"查询动态"}, rule=to_me(), priority=5, block=True
)  # 数值越小优先级越高
vive.__doc__ = "查看动态"
...
async def _(
    ...
    if not (uid := await uid_extract(name)):
        return await vive.send(
            MessageSegment.at(event.user_id) + " 未找到该 UP，请输入正确的UP 名、UP UID或 UP 首页链接"
        )
    ...
    if dynamics:
        ...
        try:
            ...
            return await vive.send(
                MessageSegment.at(event.user_id) + " 你输入的数字过大，该 UP 的最后一页动态没有这么多条"
            )
        ...
        if dyn["type"] == "DYNAMIC_TYPE_AV":  # 视频动态直接放视频链接，动态里的视频框在App上视频点不动（可能是B站bug）
            jump_url: str = dyn["modules"]["module_dynamic"]["major"]["archive"][
                "jump_url"
            ]
