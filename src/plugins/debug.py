...
debug = on_command(
    "debug", permission=SUPERUSER | GROUP_ADMIN | GROUP_OWNER, priority=2, block=True
)


@debug.handle()
async def _(
    event: GroupMessageEvent, bot: Bot, matcher: Matcher, argMsg: Message = CommandArg()
):
    ...
    if event.reply:
        ...
        if arg == "echo":
            ...
            await matcher.finish(
                f"{nickname}\n{nickname.encode('unicode_escape')}"
                if nickname
                else "获取昵称失败"
            )
        else:
