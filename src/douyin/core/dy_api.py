...
async def get_live_state_json(room_id):
    ...
    async with AsyncClient() as client:
        req = await client.get(api_url, headers=get_request_headers(), timeout=10)
    res = req.text
...
async def get_web_rid_from_short_url(url: str):
    async with AsyncClient() as client:
        resp = await client.head(url, headers=get_request_headers())
    ...
    async with AsyncClient() as client:
        resp = await client.get(api, headers=get_request_headers())
...
async def get_user_info(sec_user_id):
    ...
    headers = {
        ...
        "Cookie": await cookie_utils.get_cookie_cache()
        + "; msToken="
        + ms_token
    ...
    async with AsyncClient() as client:
        resp = await client.get(prefix + params, headers=headers)
