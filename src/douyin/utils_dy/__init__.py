...
from ..utils_dy.web_rid import get_live_room_id, get_sec_user_id_from_live_url
...
async def get_sec_user_id(user_name: str) -> Union[Optional[str], List[Any]]:
    ...
    url = (
        "https://www.douyin.com/aweme/v1/web/discover/search/?device_platform=webapp&aid=6383&channel=channel_pc_web&search_channel=aweme_user_web&keyword="
        + user_name_encoded
    ...
    try:
        async with AsyncClient() as client:
            resp = await client.get(
                url,
                headers=headers,
                cookies=get_dict_cookies_from_file(),
            )
        resp.encoding = "utf-8"
...
async def get_user_dynamics(sec_user_id: str, name: str):
    ...
    try:
        ...
        logger.error(
            f"获取抖音用户动态失败，可能是cookie 或 X-Bogus失效: {e} id:{name}, empty resp:{resp_is_empty}"
        )
        return None
...
async def get_aweme_short_url(aweme_id) -> Optional[str]:
    """获取视频短链"""
    url = (
        f"https://www.douyin.com/aweme/v1/web/web_shorten/?target=https://www.iesdouyin.com/share/video/{aweme_id}/"
...
async def create_aweme_msg(dyn: Any) -> Message:
    ...
    dyn["region"]
    dyn["duration"]
    dyn["statistics"]  # comment_count, digg_count, collect_count
    dyn["status"]["allow_share"]
