...
async def _(
    ...
    if is_short_url:  # 存在直播间短链的情况下，优先使用从短链获取的数据（抖音允许重名，短链分享不会获取到重名用户）
        ...
        if not (sec_uid := await get_sec_user_id(user_name=user_name)):
            return await add_sub_dy.send(
                MessageSegment.at(event.user_id) + " 未找到该抖音用户名"
            )
    ...
    result_exists = await db.get_sub_dy(
        group_id=event.group_id, bot_id=int(bot.self_id), sec_uid=sec_uid
    )

    if result_exists:
        await db.update_sub_dy(
            name=user_name,
