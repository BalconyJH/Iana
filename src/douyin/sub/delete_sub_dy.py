...
async def _(
    ...
    if user_name.startswith("MS4wLj") and len(user_name) > 50:  # 用户输入了一个 sec_uid
        ...
        if not user:
            return await delete_sub_dy.send(
                MessageSegment.at(event.user_id) + " 未找到该 UP，请输入正确的UP 名、sec_uid"
            )
        sec_uid = user.sec_uid

    result = await db.delete_sub_dy(
        sec_uid=sec_uid, group_id=event.group_id, bot_id=int(bot.self_id)
    )
